...
Abra a pasta do backend (Api_Crud) no VS Code.

Abra o terminal do VS Code (Ctrl + ') e rode este comando:

Bash
npm install
O que acontece? O sistema vai ler o projeto e baixar automaticamente todas as dependências necessárias (Prisma, Express, Cors, etc).

Passo 3: O Cabo de Força (Arquivo .env)
O projeto utiliza o Aiven, que é um banco de dados hospedado na nuvem.

Crie um arquivo chamado .env na raiz da pasta Api_Crud.

Cole exatamente a linha abaixo lá dentro (ela já contém as credenciais de acesso à nuvem):
...
Escreva o modelo da tabela no schema.prisma (ex: model Produtos { ... }).

Salve o arquivo.

No terminal (CMD) do VS Code, rode:
...
Vá no arquivo schema.prisma e apague o bloco da tabela inteira que deseja remover.

Salve o arquivo.

No terminal (CMD) do VS Code, rode o mesmo comando:
